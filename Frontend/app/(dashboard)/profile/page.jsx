    ...
    const hydrateProfile = async () => {
      try {
        ...
        const nextProfile = {
          ...
          userRole: rawRole,
          tags: Array.isArray(payload.badges)
            ? payload.badges.map((b) => b.label)
            : [],
      ...
      {showLogoutModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm px-4">
          <div className="w-full max-w-sm rounded-2xl border border-[#D6E3F5]/20 bg-[#101723] p-6 text-center shadow-2xl">
            ...
            <p className="mb-6 text-sm text-[#ADC7EB]">
          ...
          <div className="rounded-2xl border border-[#D6E3F5]/20 bg-[#101723] p-6">
            <div className="flex flex-col gap-6 md:flex-row md:items-center">
              ...
              <div>
                ...
                <p className="text-xl font-medium text-[#0950C3]">
                  {profile.userId}
                </p>
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  {profile.tags.map((tag, i) => (
                    <span
                      key={i}
                      className="rounded-lg bg-[#CEE0FD] px-3 py-1 text-xs font-bold text-[#0950C3]"
                    >
                      {tag}
                    </span>
                  ))}
          ...
          <div className="space-y-6">
            ...
            <div className="rounded-2xl border border-[#D6E3F5]/20 bg-[#101723] p-6">
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                {[
                  {
                    label: isInstructor ? "Assigned Courses" : "Courses",
                    val: profile.courses,
                    color: "text-[#09C398]",
                  },
                  {
                    label: isInstructor
                      ? "Student Avg Progress"
                      : "Avg Progress",
                    val: `${profile.avgProgress}%`,
                    color: "text-[#0950C3]",
                  },
                  {
                    label: isInstructor ? "Published Lessons" : "Certificates",
                    val: profile.certificates,
                    color: "text-[#DE55F3]",
                  },
                  {
                    label: isInstructor ? "Avg Rating" : "Avg Score",
                    val: profile.avgScore,
                    color: "text-[#EF9B15]",
                  },
                ].map((stat, i) => (
                  <div
                    key={i}
                    className="rounded-xl border border-[#D6E3F5]/10 p-5 text-center bg-[#161f2c]"
                  >
                    <div className={`text-2xl font-bold ${stat.color}`}>
                      {stat.val}
                    </div>
                    <div className="text-sm text-[#D6E3F5] opacity-80">
                      {stat.label}
                    </div>
                  </div>
                ))}
                ...
                <div className="space-y-3">
                  {profile.achievements.length > 0 ? (
                    profile.achievements.map((ach, index) => (
                      <div
                        key={index}
                        className="flex items-center gap-4 rounded-xl border border-[#D6E3F5]/10 bg-[#161f2c] px-4 py-4 hover:border-blue-500/50 transition-all"
                      >
                        ...
                        <div>
                          ...
                          <div className="text-sm text-[#ADC7EB]">
                            {ach.description}
                          </div>
                        </div>
                      </div>
                    ...
                    <p className="text-sm text-[#ADC7EB] italic">
