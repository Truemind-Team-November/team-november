...
export default function CourseDetailsContent() {
  ...
  const progressValue = useMemo(
    () =>
      Math.max(
        0,
        Math.min(100, Math.round(Number(course?.progressPercentage || 0))),
      ),
    [course],
  );
        ...
        <div className="grid lg:grid-cols-[2fr_1fr] gap-6">
          <section className="border border-slate-400 rounded-2xl p-6">
            ...
            <div className="flex flex-wrap gap-3 mb-5">
              ...
              <span className="bg-zinc-700/30 text-zinc-200 text-xs font-bold px-3 py-1 rounded-full border border-zinc-600/40">
            ...
            {Array.isArray(course.modules) && course.modules.length > 0 ? (
              <div className="space-y-3">
                {course.modules.map((module) => (
                  <div
                    key={module.lessonId}
                    className="border border-slate-500/40 rounded-xl p-4"
                  >
                    ...
                    <p className="text-xs text-zinc-500 mt-2">
                      Estimated: {module.estimatedMinutes || 0} mins • Contents:{" "}
                      {module.contentCount}
                    </p>
                  </div>
                ))}
          ...
          <aside className="border border-slate-400 rounded-2xl p-6 space-y-4">
            ...
            {course.resumeLessonId && (
              <Link
                href={`/lesson?lessonId=${course.resumeLessonId}`}
