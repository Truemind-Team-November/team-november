...
export default function Assignments() {
  ...
  const loadAssignments = useCallback(async () => {
    try {
      ...
      const statusQuery =
        activeCategory === "All"
          ? ""
          : `?status=${encodeURIComponent(activeCategory)}`;
  ...
  const handleSubmitAssignment = async (assignment) => {
    ...
    if (!answer && !file) {
      setSubmitFeedback(
        assignmentId,
        "error",
        "Add an answer or attachment before submitting.",
      );
      return;
    }
    ...
    try {
      ...
      const message =
        ...
        "Unable to submit assignment right now.";
  ...
  return (
    <main
      ...
      className="min-h-screen text-zinc-300 font-sans p-4 md:p-8 overflow-y-auto"
    >
      ...
      {error && (
        <div className="rounded-2xl border border-red-500/40 p-6 text-sm text-red-300">
          {error}
        </div>
      )}
      ...
      {!loading && !error && visibleAssignments.length === 0 && (
        <div className="rounded-2xl border border-slate-400 p-10 text-center">
          ...
          <p className="text-sm text-zinc-400">
            Assignments in this category will appear here once available.
          </p>
        </div>
      )}
      ...
      {!loading && !error && visibleAssignments.length > 0 && (
        <div className="space-y-6">
          {visibleAssignments.map((item) => {
            ...
            const draft = submissionDrafts[item.assignmentId] || {
              answer: "",
              file: null,
            };
            ...
            return (
              <section
                ...
                className="border border-slate-400 rounded-2xl p-6 bg-transparent w-full"
              >
                <div className="flex justify-between items-start mb-4 gap-4">
                  ...
                  <span
                    className={`text-[10px] font-bold px-4 py-1.5 rounded-full ${badgeClass}`}
                  >
                    {status}
                ...
                {canSubmit && (
                  <div className="mt-6 border border-zinc-700/60 rounded-xl p-4">
                    ...
                    <textarea
                      value={draft.answer}
                      onChange={(e) =>
                        setDraftValue(item.assignmentId, {
                    ...
                    <label className="block text-xs text-zinc-300 mt-4 mb-2">
                      Attachment (optional)
                    </label>
                    <input
                      type="file"
                    ...
                    {submitState?.text && (
                      <p
                        className={`mt-3 text-xs ${submitState.type === "error" ? "text-red-300" : submitState.type === "success" ? "text-green-300" : "text-zinc-300"}`}
                      >
                        {submitState.text}
                      </p>
                    )}
