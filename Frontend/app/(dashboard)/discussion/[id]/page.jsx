...
export default function DiscussionThread() {
  ...
  const handleReply = async () => {
    ...
    try {
      const res = await client.post(`/Discussion/${id}/reply`, {
        content: contentToPost,
      });
      if (res.data?.success) {
  ...
  if (loading) {
    return (
      <main
        ...
        className="min-h-screen text-zinc-300 font-sans p-4 md:p-8 flex items-center justify-center"
      >
  ...
  if (error || !thread) {
    return (
      <main
        style={{ backgroundColor: ThemeColors.bgBlue }}
        className="min-h-screen text-zinc-300 font-sans p-4 md:p-8 flex flex-col items-center justify-center"
      >
        <span className="text-red-400 font-medium mb-4">
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-4 mb-8">
        <button
          onClick={() => router.back()}
          className="text-zinc-400 hover:text-white transition-colors flex items-center gap-2 font-medium bg-transparent border-none cursor-pointer"
        >
          ...
          <div className="flex gap-4 md:gap-5">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-xs text-[#7D7F82] font-medium">
              ...
              <div className="flex flex-wrap items-center gap-3">
                {post.tags?.map((tag, j) => {
                  ...
                  return (
                    <span
                      key={tag.id || j}
                      className={`${color.bg} ${color.text} text-[11px] font-bold px-3 py-1 rounded-full`}
                    >
                      {tag.name}
                    </span>
                  );
        ...
        <div className="space-y-6 pt-4">
          ...
          <div className="space-y-4">
            {replies?.map((reply) => (
              <div
                key={reply.id}
                className="border border-[#7D7F82]/20 rounded-2xl p-5 md:p-6 bg-transparent ml-4 md:ml-12 flex gap-4 transition-colors hover:border-[#7D7F82]/40"
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-2">
                    <span className="font-bold text-[14px] text-zinc-100">
                      {reply.authorName}
                    </span>
                    <span className="text-xs text-[#7D7F82] font-medium">
            ...
            {(!replies || replies.length === 0) && (
              <div className="ml-4 md:ml-12 py-8 text-center border border-dashed border-[#7D7F82]/30 rounded-2xl">
                <p className="text-zinc-500 text-sm italic">
                  No responses yet. Be the first to spark the conversation!
                </p>
              </div>
            )}
          </div>
